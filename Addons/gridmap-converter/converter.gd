...
func convert(gridmap: GridMap, use_multimesh: bool) -> Node3D:
	...
	parent.transform = gridmap.transform
	...
	print("use_multimesh: ", use_multimesh)
	...
	if use_multimesh:
		...
		for mesh_id in mesh_id_to_cells:
			print("Processing mesh_id: ", mesh_id)  # ← add this as the very first line
			...
			var item_transform = mesh_lib.get_item_mesh_transform(mesh_id)
			print("item_transform: ", item_transform)
			print("basis scale: ", item_transform.basis.get_scale())
			...
			for i in range(cells.size()):
				...
				# Apply item scale to the basis, keep cell position
				var final_basis = basis * item_transform.basis
				var final_origin = cell_position + basis * item_transform.origin
				multi_mesh.set_instance_transform(i, Transform3D(final_basis, final_origin))
		...
		for cell in used_cells:
			...
			# Get the mesh library item's baked transform (includes your 0.25 scale)
			var item_transform = mesh_lib.get_item_mesh_transform(mesh_id)
			
			var final_basis = basis * item_transform.basis
			var final_origin = cell_position + basis * item_transform.origin
			mesh_instance.transform = Transform3D(final_basis, final_origin)
